    ...
    if request.method=='PUT':
        ...
        if UserAccount.objects.get(email=email): #see if email exists in db
            ...
            if check_password(password, user.password): #check if passwords match
                return JsonResponse({'id': user.id, 'name': user.name, 'username': user.username, 'email': user.email, 'hasRead': user.hasRead, 'isReading': user.isReading, 'toRead': user.toRead}) #if passwords match, return a user dict
